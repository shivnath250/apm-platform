      ...
      {rows.map((r, i) => (
        <div key={r.eid} className={`alert-row ${r.status}`} style={{ animationDelay: `${Math.min(i * 0.03, 0.4)}s` }} onClick={() => onSelectEquip(r.eid)}>
          ...
          <div className="alert-driver">
            {r.worst && (<><span className="dlabel">{r.worst.label}</span><span className="dval">{r.worst.value?.toFixed(1)} {r.worst.unit}</span></>)}
          </div>
  ...
  return (
    <div>
      ...
      <div className="detail-head">
        <h2>Alert Center</h2>
        <div className="empty-hint" style={{ margin: 0 }}>Every alarm and warning across all 12 plants. Click a row to open the equipment; click a plant name to open that plant.</div>
      </div>
      <div className="filter-bar">
        <button className={`seg ${filter === 'all' ? 'on' : ''}`} onClick={() => onFilter('all')}>Both · {rows.length}</button>
        <button className={`seg ${filter === 'alarm' ? 'on' : ''}`} onClick={() => onFilter('alarm')}>Alarms · {alarms.length}</button>
        <button className={`seg ${filter === 'warning' ? 'on' : ''}`} onClick={() => onFilter('warning')}>Warnings · {warnings.length}</button>
      </div>
