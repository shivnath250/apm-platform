    ...
    <div>
      ...
      <div className="detail-head">
        ...
        <div className="empty-hint" style={{ margin: 0 }}>
          {plant.cap} MW · {items.length} assets · <span className="alarm-t">{alarms} alarms</span>,{' '}
          <span className="warning-t">{critical.length - alarms} warnings</span>
        </div>
      </div>
      <div className="filter-bar">
        <button className={`seg ${filter === 'critical' ? 'on' : ''}`} onClick={() => setFilter('critical')}>Critical equipment · {critical.length}</button>
        <button className={`seg ${filter === 'all' ? 'on' : ''}`} onClick={() => setFilter('all')}>All equipment · {items.length}</button>
      </div>
      {shown.length === 0 ? (
