...
const PALETTE = ['#21d4c4', '#d6a52a', '#f85149', '#7aa2f7', '#bb9af7', '#9ece6a', '#ff9e64', '#2ac3de', '#e0af68', '#f7768e']
const MAXPTS = 320
...
export default function EquipmentDetail({ db, eid, weights, tree, onBack, onBackPlant, liveTickId, liveSensors, liveTs, liveOn }) {
  ...
  useEffect(() => {
    ...
    const map = new Map()
    for (const r of rows) {
      if (!map.has(r.skey)) map.set(r.skey, { skey: r.skey, label: r.label, unit: r.unit, baseline: r.baseline, healthy_max: r.hmax, series: [] })
  ...
  useEffect(() => {
    ...
    setSensors((prev) => prev.map((s) => {
      ...
      if (v == null) return s
      const series = [...s.series, { t: label, v }]
        ...
        <ResponsiveContainer width="100%" height={230}>
          <AreaChart data={hSeries} margin={{ top: 8, right: 14, left: -14, bottom: 0 }}>
            <defs><linearGradient id="hgrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#21d4c4" stopOpacity={0.35} /><stop offset="100%" stopColor="#21d4c4" stopOpacity={0} />
            </linearGradient></defs>
            ...
            <Area type="monotone" dataKey="health" stroke="#21d4c4" strokeWidth={2} fill="url(#hgrad)" isAnimationActive={false} />
          </AreaChart>
        </ResponsiveContainer>
        ...
        <div className="chart-card wide">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={combined} margin={{ top: 8, right: 14, left: -14, bottom: 0 }}>
              <CartesianGrid stroke="#1f2733" vertical={false} />
              <XAxis dataKey="t" tick={{ fill: '#5b6675', fontSize: 9 }} minTickGap={60} />
              <YAxis tick={{ fill: '#5b6675', fontSize: 9 }} width={42} tickFormatter={(v) => `${v}%`} domain={['auto', 'auto']} />
