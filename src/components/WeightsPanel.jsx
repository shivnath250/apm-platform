...
export default function WeightsPanel({ plants, weightsByPlant, defaults, onChange, onClose }) {
  ...
  const set = (k, v) => onChange({ ...weightsByPlant, [pid]: { ...current, [k]: v } })
  const reset = () => onChange({ ...weightsByPlant, [pid]: { ...(defaults.byPlant[pid] || {}) } })
  const copySql = () => {
    const sql = keys.map((k) => `UPDATE weight SET weight = ${current[k]} WHERE plant_id = '${pid}' AND skey = '${k}';`).join('\n')
  ...
  return (
    <>
      ...
      <div className="panel-slide">
        ...
        <div className="hint">Each sensor's importance in the health score, set <b>per plant</b>. 0 = context only. Changes apply to the chosen plant instantly.</div>
        ...
        {keys.map((k) => (
          <div className="wrow" key={k}>
            <div className="wlab"><span>{labels[k] || k}</span><span className="wval">{Number(current[k]).toFixed(1)}</span></div>
            <input type="range" min="0" max="3" step="0.1" value={current[k]} onChange={(e) => set(k, parseFloat(e.target.value))} />
          </div>
        ))}
